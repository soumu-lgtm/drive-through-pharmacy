/* =============================================================
   クリニック予約システム プロトタイプ2 ── 同期ストア（ローカル/Supabase 両対応）
   -------------------------------------------------------------
   ■ 設計方針（佐川案＝幹からの継続）
...
const Store = (() => {
  ...
  // 暗号的に安全な予約番号（西村案の Math.random 予測可能を是正）
  function genCode() {
  ...
  const ready = (async () => {
    if (USE_SUPABASE) {
      try {
        ...
        console.warn("[プロトタイプ2] Supabase未接続のためローカル同期にフォールバックします（テーブル未作成の可能性）:", e && e.message);
      }
    }
