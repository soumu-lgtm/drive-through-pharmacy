...
const ReceiptChecker = (() => {
  ...
  // ============================================================
  // ★v0.17 ①適応症突合（薬と病名の対応表 drug_indication.json）
  //   院内処方(IY)の薬に対し、患者傷病名の中に当院対応表の適応病名が
  //   1つも無ければ「適応確認」をmid表示。誤検知ゼロ方針:
  //     - 対応表に無い薬はチェックしない
  //     - 受理語は広めに展開済（括弧内例示も採用）＝正当な処方を誤警告しない
  //     - 断定でなく「確認」表示（名称照合の限界を明示）
  // ============================================================
  function checkDrugIndication(r) {
    if (!MasterLoader.getDrugIndicationByName) return;
    if (!r.diseases || r.diseases.length === 0) return; // 病名ゼロは別チェックが高警告済
    const seen = new Set();
    for (const p of r.procedures) {
      if (!p.isDrug || !p.name) continue;
      const entry = MasterLoader.getDrugIndicationByName(p.name);
      if (!entry) continue;                 // 対応表に無い薬はスキップ
      if (seen.has(entry.name)) continue;
      seen.add(entry.name);
      const matched = r.diseases.some(d =>
        MasterLoader.diseaseMatchesIndication(d.name, entry));
      if (!matched) {
        const sample = (entry.adopted || []).slice(0, 3).join('、');
        r.warnings.push({
          severity: 'low',   // 名称照合の限界（咽頭炎↔上気道炎の語揺れ等）を踏まえ確認喚起に留める
          message: '適応確認: ' + p.name + ' の適応病名が見当たりません' +
                   (sample ? '（当院対応表の例: ' + sample + '…）' : '')
        });
      }
    }
  }
  ...
  // ============================================================
  // ★v0.17 ②診察料加算（consultation_add.json）
  //   (a) 併算定不可（官報明記）: 時間外/休日/深夜等の重複、乳幼児加算×時間外等、
  //       電子的診療情報連携体制整備加算×明細書発行体制等加算 → high
  //   (b) 算定もれ（当院標準加算の欠落）→ low リマインド（収益機会・断定しない）
  //   ※誤検知ゼロ: 同日限定の背反は「初診/再診が単一受診(実日数実質1)のときのみ」判定
  // ============================================================
  function checkConsultationAdd(r) {
    const ca = MasterLoader.getConsultAdd && MasterLoader.getConsultAdd();
    if (!ca) return;
    ...
    const hasName = (sub) => procNames.some(nm => nm.indexOf(sub) !== -1);
    const hasExact = (nm) => procNames.some(x => x === nm);
    const singleVisit = visitCount <= 1; // 同日限定背反を安全に判定できる条件

    // (a-1) 時間外・休日・深夜・夜間早朝等の併算定不可（最も高い1つのみ）
    //   ※初診料/再診料の時間帯加算のみ対象（名称に「初診」or「再診」を含む行）。
    //     処方料/手術/検査の時間帯加算や、正当な「乳幼児○○加算（初診）」の単独使用は誤検知しない。
    const et = ca.exclusive_time;
    if (et && Array.isArray(et.members)) {
      const types = new Set();
      for (const nm of procNames) {
        if (nm.indexOf('初診') === -1 && nm.indexOf('再診') === -1) continue;
        for (const m of et.members) {
          if (nm.indexOf(m) !== -1) { types.add(m); break; }
        }
      }
      if (types.size >= 2 && (!et.same_day_only || singleVisit)) {
        r.warnings.push({
          severity: et.severity || 'high',
          message: '併算定不可: ' + [...types].join('・') + ' が併記されています（' + (et.msg || '') + '）'
        });
      }
    }

    // (a-2) ペア併算定不可（電子的診療情報連携体制整備加算×明細書発行体制等加算 等）
    for (const pair of (ca.exclusive_pairs || [])) {
      const hasA = (pair.a || []).some(s => hasName(s));
      const hasB = (pair.b || []).some(s => hasName(s));
      if (hasA && hasB && (!pair.same_day_only || singleVisit)) {
        r.warnings.push({ severity: pair.severity || 'high', message: '併算定不可: ' + (pair.msg || '') });
      }
    }

    // (b) 当院標準加算の算定もれ（low リマインド）
    //   trigger_exact＝完全一致（電話等再診料/外来診療料は除外。電話再診は加算算定不可）
    const std = ca.clinic_standard || {};
    for (const key of ['初診', '再診']) {
      const rule = std[key];
      if (!rule) continue;
      const triggered = (rule.trigger_exact || []).some(t => hasExact(t));
      if (!triggered) continue;
      const missing = (rule.expect || [])
        .filter(x => !(x.match || []).some(m => hasName(m)))
        .map(x => x.name);
      if (missing.length) {
        r.warnings.push({
          severity: 'low',
          message: '算定もれ確認(' + key + '): ' + missing.join('、') + ' が見当たりません（当院標準加算・要否をご確認ください）'
        });
      }
    }
  }
