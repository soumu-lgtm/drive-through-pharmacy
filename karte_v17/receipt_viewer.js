...
function parseUKE(text, fileType) {
  ...
  for (let line of lines) {
    ...
    switch (recType) {
      ...
      case 'CO':
        if (currentReceipt) {
          // ★v0.16 別表Ⅰでコメントコードの公式表示文言・所属診療行為をエンリッチ
          const coCode = fields[3] || '';
          const off = MasterLoader.getBeppyoComment ? MasterLoader.getBeppyoComment(coCode) : null;
          currentReceipt.comments.push({
            identifier: fields[1] || '',
            code: coCode,
            text: fields.slice(4).join(',') || '',
            official: off ? { disp: off.d, proc: off.p, kubun: off.k } : null
          });
        }
        break;
...
// ===== 摘要（コメント）＋別表Ⅰ記載事項リファレンス（v0.16） =====
//  ・記録済み摘要(CO)を別表Ⅰ公式表示文言でエンリッチ表示（現行v0.15は摘要を一切表示していなかった）
//  ・請求された診療行為の別表Ⅰ記載事項を present(✓)/未記載(⚠無条件)/条件付き(ℹ) で照合
//  ※誤検知ゼロ方針: 「未記載」は無条件の記載事項のみ注意喚起。条件付きは該当時のみのため件数表示に留める。
function renderMoshiyoku(r) {
  const el = document.getElementById('detailMoshiyoku');
  if (!el) return;
  ...
  // --- A. 記録されている摘要（CO） ---
  html += '<div style="font-weight:700;color:var(--rc-teal,#0e7c66);margin:2px 0 6px;">記録されている摘要（' + (r.comments ? r.comments.length : 0) + '件）</div>';
  if (!r.comments || r.comments.length === 0) {
    html += '<div style="color:#888;font-size:12px;margin-bottom:10px;">このレセプトに摘要コメント（COレコード）はありません。</div>';
  } else {
    html += '<div style="margin-bottom:10px;">';
    r.comments.forEach(c => {
      const shown = c.text || (c.official ? c.official.disp : '') || '';
      const meta = c.official ? ('<span style="color:#888;font-size:11px;">〔' + esc(c.official.proc) + '〕</span>') : '';
      const codeb = c.code ? '<span style="font-family:monospace;color:#999;font-size:11px;">' + esc(c.code) + '</span> ' : '';
      html += '<div style="font-size:12.5px;line-height:1.6;border-left:3px solid var(--rc-teal,#0e7c66);padding:2px 8px;margin:3px 0;background:rgba(14,124,102,0.03);">' +
              codeb + esc(shown) + ' ' + meta + '</div>';
    });
    html += '</div>';
  }

  // --- B. 別表Ⅰ 記載事項チェック（請求された診療行為ごと） ---
  //   誤検知ゼロ方針: 記載事項の多くは「条件該当時のみ」「加算算定時のみ」「択一選択肢」であり、
  //   非記録＝漏れとは限らない。よって既定表示は「✓記録済み」＋件数サマリーのみ。
  //   未記録の記載事項は折りたたみ(<details>)の中立リファレンスとして参照可能にする（警告列挙にしない）。
  if (MasterLoader.getBeppyoRulesByProc) {
    const seenKubun = new Set();
    const cards = [];
    (r.procedures || []).forEach(p => {
      if (p.isDrug || !p.code) return;
      const ref = MasterLoader.getBeppyoRulesByProc(p.code);
      if (!ref || seenKubun.has(ref.kubun)) return;
      seenKubun.add(ref.kubun);
      const present = [], other = [];
      ref.groups.forEach(g => {
        g.items.forEach(it => {
          if (!/^\d{9}$/.test(it.code)) return;
          if (coSet.has(it.code)) present.push(it);
          else {
            const cond = (it.cond || '').trim();
            const conditional = (cond[0] === '（' || cond[0] === '(');
            other.push({ disp: it.disp, gname: g.name, conditional });
          }
        });
      });
      const total = present.length + other.length;
      if (total === 0) return;
      const name = ref.groups[0].name;
      let card = '<div style="border:1px solid var(--rc-border,#d9d4c8);margin:5px 0;padding:6px 10px;font-size:12px;line-height:1.6;">' +
                 '<div style="font-weight:700;">' + esc(name) +
                 ' <span style="color:#999;font-weight:400;font-size:11px;">区分 ' + esc(ref.kubun) + '</span>' +
                 ' <span style="color:#666;font-weight:400;font-size:11px;">記載事項 全' + total + '件・記録済 ' + present.length + '件</span></div>';
      present.forEach(it => {
        card += '<div style="color:var(--rc-green,#3a6b35);">&#10003; ' + esc(trunc(it.disp, 52)) + '</div>';
      });
      if (other.length) {
        card += '<details style="margin-top:3px;"><summary style="cursor:pointer;color:#777;font-size:11.5px;">未記録の記載事項 ' + other.length + '件を表示（該当する状況・加算のときのみ必要）</summary>' +
                '<div style="margin-top:3px;padding-left:6px;border-left:2px solid var(--rc-border,#d9d4c8);">';
        other.forEach(o => {
          const tag = o.conditional
            ? '<span style="color:#999;font-size:10.5px;">［条件付］</span> '
            : '<span style="color:#999;font-size:10.5px;">［無条件］</span> ';
          card += '<div style="color:#888;font-size:11.5px;">' + tag + esc(trunc(o.disp, 52)) + '</div>';
        });
        card += '</div></details>';
      }
      card += '</div>';
    ...
    html += '<div style="font-weight:700;color:var(--rc-teal,#0e7c66);margin:6px 0;">別表Ⅰ 記載事項チェック（厚労省 摘要欄記載事項一覧）</div>';
    if (cards.length === 0) {
      html += '<div style="color:#888;font-size:12px;">請求された診療行為に別表Ⅰの記載事項ルールはありません。</div>';
    } else {
      html += cards.join('');
      html += '<div style="color:#999;font-size:11px;margin-top:4px;">※ ✓＝当レセプトの摘要に記録済み。未記録分は条件該当・加算算定・択一選択のときのみ必要で、漏れとは限りません。出典: 厚労省告示 別表Ⅰ（医科）。</div>';
    }
  }
  el.innerHTML = html;
}
