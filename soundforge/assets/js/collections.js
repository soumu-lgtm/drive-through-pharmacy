/**
 * SoundForge コレクションデータ — 自動生成 by build_collection_pages.py
 */
const COLLECTIONS = {
  "trpg": {
    "id": "trpg",
    "title": "ファンタジー/ストーリー動画 BGM＆背景セット",
    "titleEn": "Fantasy & Story Video Pack",
    "description": "クトゥルフ・ダークファンタジー・ストーリー動画向け素材集",
    "image_subdir": "trpg",
    "scenes": [
      {
        "id": "coc_old_library",
        "name": "古い図書館",
        "nameEn": "Old Library",
        "desc": "1920年代の埃積もる書斎。古文書と魔導書が眠る場所",
        "bgmStyle": "ダークアンビエント / オルガン",
        "audio_folder": "trpg_coc_exploration",
        "category": "CoC探索",
        "image": "coc_old_library.png",
        "image_subdir": "trpg",
        "audio": "antique_shop_twilight_v1.mp3",
        "status": "ready"
      },
      {
        "id": "coc_victorian_mansion",
        "name": "ヴィクトリア朝の館",
        "nameEn": "Victorian Mansion",
        "desc": "夜のゴシック邸宅。ろうそく揺れる長い廊下",
        "bgmStyle": "ストリングス / ミステリアス",
        "audio_folder": "trpg_coc_exploration",
        "category": "CoC探索",
        "image": "coc_victorian_mansion.png",
        "image_subdir": "trpg",
        "audio": "beneath_the_chapel_v1.mp3",
        "status": "ready"
      },
      {
        "id": "coc_rainy_street_1920s",
        "name": "雨の街路（1920年代）",
        "nameEn": "Rainy 1920s Street",
        "desc": "ガス灯灯る石畳の濡れた路地。ノワール調",
        "bgmStyle": "ジャジー・サスペンス",
        "audio_folder": "trpg_coc_exploration",
        "category": "CoC探索",
        "image": "coc_rainy_street_1920s.png",
        "image_subdir": "trpg",
        "audio": "dust_of_the_library_v1.mp3",
        "status": "ready"
      },
      {
        "id": "coc_underground_tunnel",
        "name": "地下トンネル",
        "nameEn": "Underground Tunnel",
        "desc": "煉瓦造りの暗渠。松明の灯りに浮かぶ謎の紋様",
        "bgmStyle": "ホラー・ドローン",
        "audio_folder": "trpg_coc_horror",
        "category": "CoC恐怖",
        "image": "coc_underground_tunnel.png",
        "image_subdir": "trpg",
        "audio": "asylum_screams_v1.mp3",
        "status": "ready"
      },
      {
        "id": "coc_abandoned_chapel",
        "name": "廃礼拝堂",
        "nameEn": "Abandoned Chapel",
        "desc": "崩れたステンドグラスから差し込む月光。退廃と恐怖",
        "bgmStyle": "聖歌 / 不協和音",
        "audio_folder": "trpg_coc_horror",
        "category": "CoC恐怖",
        "image": "coc_abandoned_chapel.png",
        "image_subdir": "trpg",
        "audio": "bell_of_the_end_v1.mp3",
        "status": "ready"
      },
      {
        "id": "fantasy_medieval_town",
        "name": "中世の街",
        "nameEn": "Medieval Town",
        "desc": "石畳の広場と木組みの家。穏やかなRPG街並み",
        "bgmStyle": "民族楽器 / 牧歌的",
        "audio_folder": "trpg_fantasy_town",
        "category": "ファンタジー街",
        "image": "fantasy_medieval_town.png",
        "image_subdir": "trpg",
        "audio": "bustling_tavern_v1.mp3",
        "status": "ready"
      },
      {
        "id": "fantasy_tavern_interior",
        "name": "酒場",
        "nameEn": "Tavern Interior",
        "desc": "暖炉が燃えるRPGタバーン。冒険者の集う場所",
        "bgmStyle": "リュート / 陽気",
        "audio_folder": "trpg_fantasy_town",
        "category": "ファンタジー街",
        "image": "fantasy_tavern_interior.png",
        "image_subdir": "trpg",
        "audio": "city_of_scholars_v1.mp3",
        "status": "ready"
      },
      {
        "id": "fantasy_crystal_cave",
        "name": "クリスタル洞窟",
        "nameEn": "Crystal Cave",
        "desc": "青く光る水晶が壁に埋め込まれた神秘の洞",
        "bgmStyle": "アンビエント・ミスティック",
        "audio_folder": "trpg_fantasy_dungeon",
        "category": "ファンタジーDG",
        "image": "fantasy_crystal_cave.png",
        "image_subdir": "trpg",
        "audio": "crystal_cave_v1.mp3",
        "status": "ready"
      },
      {
        "id": "fantasy_ancient_ruins",
        "name": "古代遺跡",
        "nameEn": "Ancient Ruins",
        "desc": "蔦に覆われた石柱。失われた文明の痕跡",
        "bgmStyle": "オーケストラ / 探索",
        "audio_folder": "trpg_fantasy_dungeon",
        "category": "ファンタジーDG",
        "image": "fantasy_ancient_ruins.png",
        "image_subdir": "trpg",
        "audio": "dragon_s_wrath_v1.mp3",
        "status": "ready"
      },
      {
        "id": "fantasy_dragon_lair",
        "name": "ドラゴンの巣",
        "nameEn": "Dragon Lair",
        "desc": "金貨と財宝が積まれた巨大洞窟。ボス戦の舞台",
        "bgmStyle": "オーケストラ / 緊迫",
        "audio_folder": "trpg_fantasy_dungeon",
        "category": "ファンタジーDG",
        "image": "fantasy_dragon_lair.png",
        "image_subdir": "trpg",
        "audio": "entrance_to_ancient_ruins_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_shrine_dawn",
        "name": "夜明けの神社",
        "nameEn": "Shrine at Dawn",
        "desc": "鳥居と朝霧の神社。侘び寂びの静謐",
        "bgmStyle": "尺八 / 琴 / アンビエント",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_shrine_dawn.png",
        "audio": "cherry_blossom_garden_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_yokai_forest",
        "name": "妖怪の森",
        "nameEn": "Forest of Yokai",
        "desc": "月光と霧に浮かぶ古竹林。妖怪伝承の闇",
        "bgmStyle": "琴ドローン / 低音太鼓",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_yokai_forest.png",
        "audio": "eve_of_battle_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_old_house_hearth",
        "name": "古民家の囲炉裏",
        "nameEn": "Hearth of the Old House",
        "desc": "畳の間、囲炉裏の火。懐かしい田舎の日本",
        "bgmStyle": "三味線 / 温かな和ambient",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_old_house_hearth.png",
        "audio": "forest_of_yokai_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_snow_mountain_pass",
        "name": "雪深い峠道",
        "nameEn": "Snow Mountain Pass",
        "desc": "雪降る山道と遠くの塔影。孤独と寂寥",
        "bgmStyle": "尺八 / 冷たいドローン",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_snow_mountain_pass.png",
        "audio": "hearth_of_the_old_house_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_battle_eve",
        "name": "合戦の前夜",
        "nameEn": "Eve of Battle",
        "desc": "戦国の陣営。旗と篝火、緊迫の前夜",
        "bgmStyle": "太鼓 / 低弦 / 和風シネマ",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_battle_eve.png",
        "audio": "kitsunebi_moonlight_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_oni_roar",
        "name": "鬼の咆哮",
        "nameEn": "Oni's Roar",
        "desc": "雷鳴の山に巨鬼の影。和風戦闘クライマックス",
        "bgmStyle": "雷鳴太鼓 / 攻撃的三味線",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_oni_roar.png",
        "audio": "oni_s_roar_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_cherry_garden",
        "name": "桜吹雪の庭園",
        "nameEn": "Cherry Blossom Garden",
        "desc": "満開の桜と池、花びら舞う和の庭",
        "bgmStyle": "琴アルペジオ / 優雅",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_cherry_garden.png",
        "audio": "shrine_at_dawn_v1.mp3",
        "status": "ready"
      },
      {
        "id": "wafu_kitsunebi_moonlight",
        "name": "月夜の狐火",
        "nameEn": "Kitsunebi Moonlight",
        "desc": "月下の森に青い狐火が漂う。神秘の夜",
        "bgmStyle": "幻想琴 / 遠い鐘",
        "audio_folder": "fantasy_wafu",
        "category": "和風ファンタジー",
        "image_subdir": "fantasy_wafu",
        "image": "wafu_kitsunebi_moonlight.png",
        "audio": "snow_mountain_pass_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_after_school",
        "name": "放課後の静けさ",
        "nameEn": "After School Calm",
        "desc": "夕日射す無人の教室。郷愁の日常",
        "bgmStyle": "Lo-Fiピアノ",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_after_school.png",
        "audio": "abandoned_school_memories_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_abandoned_school",
        "name": "廃校の記憶",
        "nameEn": "Abandoned School Memories",
        "desc": "割れた窓と埃。都市伝説の舞台",
        "bgmStyle": "ミュートピアノ / 不穏",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_abandoned_school.png",
        "audio": "after_school_calm_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_back_alley",
        "name": "都市の裏路地",
        "nameEn": "City Back Alley",
        "desc": "ネオンと雨の裏路地。現代伝奇の暗部",
        "bgmStyle": "ダークアーバン / Lo-Fiベース",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_back_alley.png",
        "audio": "barrier_forest_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_barrier_forest",
        "name": "結界の森",
        "nameEn": "Barrier Forest",
        "desc": "青い光の結界が張る幻想の森",
        "bgmStyle": "シンセパッド / チャイム",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_barrier_forest.png",
        "audio": "city_back_alley_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_power_awakening",
        "name": "異能発動",
        "nameEn": "Power Awakening",
        "desc": "都市夜景と魔法陣。異能バトルの瞬間",
        "bgmStyle": "シネマティック / アニメ戦闘",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_power_awakening.png",
        "audio": "dream_boundary_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_midnight_conbini",
        "name": "深夜のコンビニ前",
        "nameEn": "Midnight Convenience Store",
        "desc": "深夜のコンビニ明かりと無人駐車場",
        "bgmStyle": "Lo-Fiシティポップ",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_midnight_conbini.png",
        "audio": "midnight_convenience_store_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_dream_boundary",
        "name": "夢と現実の境界",
        "nameEn": "Dream Boundary",
        "desc": "パステル空と浮遊島。世界の境",
        "bgmStyle": "ドリーミーambient / オルゴール",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_dream_boundary.png",
        "audio": "power_awakening_v1.mp3",
        "status": "ready"
      },
      {
        "id": "modern_rooftop_twilight",
        "name": "黄昏の屋上",
        "nameEn": "Rooftop Twilight",
        "desc": "夕焼けの校舎屋上。青春のほろ苦さ",
        "bgmStyle": "モダンオーケストラ / ピアノ",
        "audio_folder": "fantasy_modern",
        "category": "学園異能",
        "image_subdir": "fantasy_modern",
        "image": "modern_rooftop_twilight.png",
        "audio": "rooftop_twilight_v1.mp3",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 26,
      "image": 26,
      "bgm": 26,
      "ready": 26
    }
  },
  "streaming": {
    "id": "streaming",
    "title": "配信待機画面等 BGM＆背景セット",
    "titleEn": "Stream Standby Pack",
    "description": "VTuber・YouTuber・Twitch向け 待機画面/開始前/雑談 BGM素材集",
    "image_subdir": "streaming",
    "scenes": [
      {
        "id": "streaming_cute_cafe",
        "name": "かわいいカフェ",
        "nameEn": "Cute Cafe",
        "desc": "パステルピンクとミントのLo-Fiカフェ。窓辺にラテ",
        "bgmStyle": "Lo-Fi カフェジャズ",
        "audio_folder": "streaming_lofi",
        "category": "待機画面",
        "image": "streaming_cute_cafe.png",
        "image_subdir": "streaming",
        "audio": "casual_chat_time_v1.mp3",
        "status": "ready"
      },
      {
        "id": "streaming_cozy_room",
        "name": "コージールーム",
        "nameEn": "Cozy Room",
        "desc": "フェアリーライトとぬいぐるみの女子部屋",
        "bgmStyle": "Lo-Fi ベッドルーム",
        "audio_folder": "streaming_lofi",
        "category": "待機画面",
        "image": "streaming_cozy_room.png",
        "image_subdir": "streaming",
        "audio": "cozy_gaming_stream_v1.mp3",
        "status": "ready"
      },
      {
        "id": "streaming_night_window_city",
        "name": "夜の窓辺の街",
        "nameEn": "Night Window City",
        "desc": "ネオン街を見下ろす雨の窓。深夜の待機画面向け",
        "bgmStyle": "Lo-Fi シティナイト",
        "audio_folder": "streaming_lofi",
        "category": "深夜配信",
        "image": "streaming_night_window_city.png",
        "image_subdir": "streaming",
        "audio": "cute_morning_greeting_v1.mp3",
        "status": "ready"
      },
      {
        "id": "streaming_pastel_kitchen",
        "name": "パステルキッチン",
        "nameEn": "Pastel Kitchen",
        "desc": "ピンクとミントの可愛いキッチン。朝の光",
        "bgmStyle": "Lo-Fi モーニング",
        "audio_folder": "streaming_lofi",
        "category": "朝配信",
        "image": "streaming_pastel_kitchen.png",
        "image_subdir": "streaming",
        "audio": "late_night_stream_work_v1.mp3",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 4,
      "image": 4,
      "bgm": 4,
      "ready": 4
    }
  },
  "story": {
    "id": "story",
    "title": "ゆっくり解説 BGM＆背景セット",
    "titleEn": "Commentary Video Pack",
    "description": "ゆっくり解説・雑学・歴史・ミステリー・ランキング動画向け素材集",
    "image_subdir": "story_yukkuri",
    "scenes": [
      {
        "id": "yukkuri_peaceful_intro",
        "name": "ほのぼの導入",
        "nameEn": "Peaceful Intro",
        "desc": "パステルの勉強机。明るいチュートリアルBGM",
        "bgmStyle": "チップチューン / マリンバ",
        "audio_folder": "story_yukkuri",
        "category": "導入・イントロ",
        "image": "yukkuri_peaceful_intro.png",
        "image_subdir": "story_yukkuri",
        "audio": "comic_beat_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_history",
        "name": "歴史を語る",
        "nameEn": "Tales of History",
        "desc": "アンティーク書庫と羊皮紙。歴史ナレーション",
        "bgmStyle": "アコギ / 弦楽 / ドキュメンタリー",
        "audio_folder": "story_yukkuri",
        "category": "解説・語り",
        "image": "yukkuri_history.png",
        "image_subdir": "story_yukkuri",
        "audio": "countdown_ranking_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_trivia_pop",
        "name": "雑学ポップ",
        "nameEn": "Trivia Pop",
        "desc": "ポップアート背景。雑学コーナーの定番",
        "bgmStyle": "チップチューン / ポップ",
        "audio_folder": "story_yukkuri",
        "category": "解説・語り",
        "image": "yukkuri_trivia_pop.png",
        "image_subdir": "story_yukkuri",
        "audio": "lab_notes_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_mystery",
        "name": "ミステリー考察",
        "nameEn": "Mystery Thoughts",
        "desc": "虫眼鏡と古写真。考察系のBGM",
        "bgmStyle": "ミステリーambient / エレピ",
        "audio_folder": "story_yukkuri",
        "category": "解説・語り",
        "image": "yukkuri_mystery.png",
        "image_subdir": "story_yukkuri",
        "audio": "mystery_thoughts_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_tension",
        "name": "緊張の場面転換",
        "nameEn": "Tension Shift",
        "desc": "赤い光と煙の演出。場面転換用",
        "bgmStyle": "ビルドアップ / ティンパニ",
        "audio_folder": "story_yukkuri",
        "category": "演出・ブリッジ",
        "image": "yukkuri_tension.png",
        "image_subdir": "story_yukkuri",
        "audio": "occult_corner_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_occult",
        "name": "オカルト解説",
        "nameEn": "Occult Corner",
        "desc": "蝋燭と魔導書。都市伝説・怖い話系",
        "bgmStyle": "オルゴール / 不気味ambient",
        "audio_folder": "story_yukkuri",
        "category": "演出・ブリッジ",
        "image": "yukkuri_occult.png",
        "image_subdir": "story_yukkuri",
        "audio": "peaceful_intro_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_countdown",
        "name": "ランキング発表",
        "nameEn": "Countdown Ranking",
        "desc": "ゲームショー風の発表台。ランキング動画向け",
        "bgmStyle": "8bitカウントダウン",
        "audio_folder": "story_yukkuri",
        "category": "バラエティ",
        "image": "yukkuri_countdown.png",
        "image_subdir": "story_yukkuri",
        "audio": "tales_of_history_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_lab_notes",
        "name": "科学・テクノロジー",
        "nameEn": "Lab Notes",
        "desc": "クリーンな研究室。科学系解説向け",
        "bgmStyle": "コーポレートambient",
        "audio_folder": "story_yukkuri",
        "category": "バラエティ",
        "image": "yukkuri_lab_notes.png",
        "image_subdir": "story_yukkuri",
        "audio": "tension_shift_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_comic_beat",
        "name": "笑いどころ",
        "nameEn": "Comic Beat",
        "desc": "コミック風スピード線。ギャグシーン",
        "bgmStyle": "ピチカート / コメディ",
        "audio_folder": "story_yukkuri",
        "category": "バラエティ",
        "image": "yukkuri_comic_beat.png",
        "image_subdir": "story_yukkuri",
        "audio": "thanks_for_watching_v1.mp3",
        "status": "ready"
      },
      {
        "id": "yukkuri_thanks",
        "name": "エンディング",
        "nameEn": "Thanks for Watching",
        "desc": "夕日の窓辺。ほっこり終幕BGM",
        "bgmStyle": "アコギ / ウクレレ / アウトロ",
        "audio_folder": "story_yukkuri",
        "category": "導入・イントロ",
        "image": "yukkuri_thanks.png",
        "image_subdir": "story_yukkuri",
        "audio": "trivia_pop_v1.mp3",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 10,
      "image": 10,
      "bgm": 10,
      "ready": 10
    }
  },
  "cyberpunk": {
    "id": "cyberpunk",
    "title": "サイバーパンク / SF・近未来 BGM",
    "titleEn": "Cyberpunk / Sci-Fi BGM",
    "description": "AI解説動画、テック系YouTube、SF系ゲーム、サイバーパンク風動画向け",
    "audio_folder": "cyberpunk_sf",
    "scenes": [
      {
        "id": "neon_night_market",
        "name": "ネオンナイトマーケット",
        "nameEn": "Neon Night Market",
        "desc": "ネオンが乱反射する闇市。違法パーツと屋台の喧騒",
        "bgmStyle": "シンセウェーブ / ダーク・エレクトロ",
        "category": "ストリート",
        "audio_v1": "neon_night_market_v1.mp3",
        "audio_v2": "neon_night_market_v2.mp3",
        "image": "neon_night_market.png",
        "status": "ready"
      },
      {
        "id": "data_flood",
        "name": "データフラッド",
        "nameEn": "Data Flood",
        "desc": "膨大なデータが流れる仮想空間。情報の洪水に溺れる",
        "bgmStyle": "グリッチホップ / IDM",
        "category": "サイバースペース",
        "audio_v1": "data_flood_v1.mp3",
        "audio_v2": "data_flood_v2.mp3",
        "image": "data_flood.png",
        "status": "ready"
      },
      {
        "id": "orbital_dawn",
        "name": "オービタル・ドーン",
        "nameEn": "Orbital Dawn",
        "desc": "宇宙ステーションから見える地球の夜明け。静寂と壮大さ",
        "bgmStyle": "スペースアンビエント / シネマティック",
        "category": "宇宙・SF",
        "audio_v1": "orbital_dawn_v1.mp3",
        "audio_v2": "orbital_dawn_v2.mp3",
        "image": "orbital_dawn.png",
        "status": "ready"
      },
      {
        "id": "heart_of_the_machine",
        "name": "機械の心臓",
        "nameEn": "Heart of the Machine",
        "desc": "巨大な機械都市の中枢。歯車とパイプが脈動する",
        "bgmStyle": "インダストリアル / メカニカル・ビート",
        "category": "サイバースペース",
        "audio_v1": "heart_of_the_machine_v1.mp3",
        "audio_v2": "heart_of_the_machine_v2.mp3",
        "image": "heart_of_the_machine.png",
        "status": "ready"
      },
      {
        "id": "rebel_algorithm",
        "name": "反逆のアルゴリズム",
        "nameEn": "Rebel Algorithm",
        "desc": "AIが自我に目覚めた瞬間。デジタルレジスタンスの始まり",
        "bgmStyle": "アグレッシブ・シンセ / テクノ",
        "category": "サイバースペース",
        "audio_v1": "rebel_algorithm_v1.mp3",
        "audio_v2": "rebel_algorithm_v2.mp3",
        "image": "rebel_algorithm.png",
        "status": "ready"
      },
      {
        "id": "ruins_of_tomorrow",
        "name": "明日の廃墟",
        "nameEn": "Ruins of Tomorrow",
        "desc": "崩壊した未来都市。錆びたビルとホログラムの残骸",
        "bgmStyle": "ポストアポカリプス・アンビエント",
        "category": "ストリート",
        "audio_v1": "ruins_of_tomorrow_v1.mp3",
        "audio_v2": "ruins_of_tomorrow_v2.mp3",
        "image": "ruins_of_tomorrow.png",
        "status": "ready"
      },
      {
        "id": "dream_of_ai",
        "name": "AIの夢",
        "nameEn": "Dream of AI",
        "desc": "人工知能が見る夢の世界。有機的で幾何学的な幻想",
        "bgmStyle": "ドリーミー・エレクトロニカ",
        "category": "宇宙・SF",
        "audio_v1": "dream_of_ai_v1.mp3",
        "audio_v2": "dream_of_ai_v2.mp3",
        "image": "dream_of_ai.png",
        "status": "ready"
      },
      {
        "id": "high_speed_chase_2099",
        "name": "ハイスピードチェイス 2099",
        "nameEn": "High Speed Chase 2099",
        "desc": "空中ハイウェイでの追跡劇。光の軌跡が交差する",
        "bgmStyle": "ドラムンベース / ハイテンポ・シンセ",
        "category": "ストリート",
        "audio_v1": "high_speed_chase_2099_v1.mp3",
        "audio_v2": "high_speed_chase_2099_v2.mp3",
        "image": "high_speed_chase_2099.png",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 8,
      "bgm": 16,
      "ready": 8
    }
  },
  "horror": {
    "id": "horror",
    "title": "ホラー・サスペンス BGM",
    "titleEn": "Horror & Suspense BGM",
    "description": "ホラーゲーム実況、心霊考察動画、怖い話、ミステリー動画向け",
    "audio_folder": "horror_suspense",
    "scenes": [
      {
        "id": "memory_of_ruins",
        "name": "廃墟の記憶",
        "nameEn": "Memory of Ruins",
        "desc": "朽ちた建物に残された誰かの痕跡。不穏な静寂",
        "bgmStyle": "ダークアンビエント / ドローン",
        "category": "探索・雰囲気",
        "audio_v1": "memory_of_ruins_v1.mp3",
        "audio_v2": "memory_of_ruins_v2.mp3",
        "image": "memory_of_ruins.png",
        "status": "ready"
      },
      {
        "id": "something_is_here",
        "name": "何かがいる",
        "nameEn": "Something Is Here",
        "desc": "暗闇の向こうに気配を感じる。振り返ってはいけない",
        "bgmStyle": "テンション・ホラー / 低音パルス",
        "category": "探索・雰囲気",
        "audio_v1": "something_is_here_v1.mp3",
        "audio_v2": "something_is_here_v2.mp3",
        "image": "something_is_here.png",
        "status": "ready"
      },
      {
        "id": "prelude_to_carnage",
        "name": "惨劇の序曲",
        "nameEn": "Prelude to Carnage",
        "desc": "嵐の前の静けさ。これから起きる惨劇の予感",
        "bgmStyle": "不協和音ストリングス / ティンパニ",
        "category": "クライマックス",
        "audio_v1": "prelude_to_carnage_v1.mp3",
        "image": "prelude_to_carnage.png",
        "status": "ready"
      },
      {
        "id": "nightmare_gate",
        "name": "悪夢の門",
        "nameEn": "Nightmare Gate",
        "desc": "現実と悪夢の境界が崩れる。異形の世界への入口",
        "bgmStyle": "リバースシンセ / ホラーパッド",
        "category": "超常現象",
        "audio_v1": "nightmare_gate_v1.mp3",
        "audio_v2": "nightmare_gate_v2.mp3",
        "image": "nightmare_gate.png",
        "status": "ready"
      },
      {
        "id": "spirit_footage_analysis",
        "name": "心霊映像解析",
        "nameEn": "Spirit Footage Analysis",
        "desc": "ノイズ混じりの映像を解析中。そこに映るものは――",
        "bgmStyle": "グリッチノイズ / 電子ホラー",
        "category": "超常現象",
        "audio_v1": "spirit_footage_analysis_v1.mp3",
        "audio_v2": "spirit_footage_analysis_v2.mp3",
        "image": "spirit_footage_analysis.png",
        "status": "ready"
      },
      {
        "id": "run_for_your_life",
        "name": "逃げろ",
        "nameEn": "Run for Your Life",
        "desc": "追われている。足音が近づく。息が止まりそうだ",
        "bgmStyle": "ハイテンポ・パニック / パーカッション",
        "category": "クライマックス",
        "audio_v1": "run_for_your_life_v1.mp3",
        "audio_v2": "run_for_your_life_v2.mp3",
        "image": "run_for_your_life.png",
        "status": "ready"
      },
      {
        "id": "midnight_ward",
        "name": "深夜の病棟",
        "nameEn": "Midnight Ward",
        "desc": "消灯後の病院。廊下に響く足音と点滅する蛍光灯",
        "bgmStyle": "ホスピタルホラー / 金属音",
        "category": "探索・雰囲気",
        "audio_v1": "midnight_ward_v1.mp3",
        "audio_v2": "midnight_ward_v2.mp3",
        "image": "midnight_ward.png",
        "status": "ready"
      },
      {
        "id": "the_ritual_night",
        "name": "儀式の夜",
        "nameEn": "The Ritual Night",
        "desc": "蝋燭に照らされた魔法陣。禁断の儀式が始まる",
        "bgmStyle": "オカルトコーラス / リチュアル・ドラム",
        "category": "超常現象",
        "audio_v1": "the_ritual_night_v1.mp3",
        "audio_v2": "the_ritual_night_v2.mp3",
        "image": "the_ritual_night.png",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 8,
      "bgm": 15,
      "ready": 8
    }
  },
  "rpg-battle": {
    "id": "rpg-battle",
    "title": "シネマティックRPGバトル BGM",
    "titleEn": "Cinematic RPG Battle BGM",
    "description": "インディーゲーム開発、ゲーム実況、PV動画、ファンタジー映像向け",
    "audio_folder": "rpg_cinematic",
    "scenes": [
      {
        "id": "return_to_the_capital",
        "name": "王都への帰還",
        "nameEn": "Return to the Capital",
        "desc": "長い旅路の果て、王都の城壁が見える。凱旋のファンファーレ",
        "bgmStyle": "オーケストラ / ファンファーレ",
        "category": "フィールド",
        "audio_v1": "return_to_the_capital_v1.mp3",
        "audio_v2": "return_to_the_capital_v2.mp3",
        "image": "return_to_the_capital.png",
        "status": "ready"
      },
      {
        "id": "blade_of_fate",
        "name": "運命の剣",
        "nameEn": "Blade of Fate",
        "desc": "宿敵との一騎打ち。剣と剣がぶつかり火花が散る",
        "bgmStyle": "シネマティック・オーケストラ / 弦楽バトル",
        "category": "バトル",
        "audio_v1": "blade_of_fate_v1.mp3",
        "audio_v2": "blade_of_fate_v2.mp3",
        "image": "blade_of_fate.png",
        "status": "ready"
      },
      {
        "id": "guardian_of_the_abyss",
        "name": "深淵の守護者",
        "nameEn": "Guardian of the Abyss",
        "desc": "地の底に潜む巨大な存在。ボス戦の緊張感",
        "bgmStyle": "ヘビーオーケストラ / コーラス",
        "category": "ボスバトル",
        "audio_v1": "guardian_of_the_abyss_v1.mp3",
        "audio_v2": "guardian_of_the_abyss_v2.mp3",
        "image": "guardian_of_the_abyss.png",
        "status": "ready"
      },
      {
        "id": "the_last_fortress",
        "name": "最後の砦",
        "nameEn": "The Last Fortress",
        "desc": "包囲された城塞。味方を守るための最終防衛戦",
        "bgmStyle": "エピック・パーカッション / 金管",
        "category": "バトル",
        "audio_v1": "the_last_fortress_v1.mp3",
        "audio_v2": "the_last_fortress_v2.mp3",
        "image": "the_last_fortress.png",
        "status": "ready"
      },
      {
        "id": "heros_proof",
        "name": "英雄の証明",
        "nameEn": "Hero's Proof",
        "desc": "試練を乗り越えた者だけが得る称号。勝利のテーマ",
        "bgmStyle": "トライアンフ・オーケストラ / ピアノ",
        "category": "フィールド",
        "audio_v1": "heros_proof_v1.mp3",
        "audio_v2": "heros_proof_v2.mp3",
        "image": "heros_proof.png",
        "status": "ready"
      },
      {
        "id": "overture_of_ruin",
        "name": "破滅の序曲",
        "nameEn": "Overture of Ruin",
        "desc": "世界が崩壊する前兆。不穏な空気が立ち込める",
        "bgmStyle": "ダーク・オーケストラ / 不協和音",
        "category": "ボスバトル",
        "audio_v1": "overture_of_ruin_v1.mp3",
        "audio_v2": "overture_of_ruin_v2.mp3",
        "image": "overture_of_ruin.png",
        "status": "ready"
      },
      {
        "id": "forgotten_temple",
        "name": "忘れられた神殿",
        "nameEn": "Forgotten Temple",
        "desc": "古代の神殿深部。神秘的な光と石碑の謎",
        "bgmStyle": "ミスティック・アンビエント / ハープ",
        "category": "ダンジョン",
        "audio_v1": "forgotten_temple_v1.mp3",
        "audio_v2": "forgotten_temple_v2.mp3",
        "image": "forgotten_temple.png",
        "status": "ready"
      },
      {
        "id": "twilight_of_the_kingdom",
        "name": "王国の黄昏",
        "nameEn": "Twilight of the Kingdom",
        "desc": "滅びゆく王国の最後の光。哀愁と決意が交差する",
        "bgmStyle": "メランコリック・オーケストラ / ピアノ",
        "category": "フィールド",
        "audio_v1": "twilight_of_the_kingdom_v1.mp3",
        "audio_v2": "twilight_of_the_kingdom_v2.mp3",
        "image": "twilight_of_the_kingdom.png",
        "status": "ready"
      },
      {
        "id": "dawn_of_the_final_battle",
        "name": "最終決戦の夜明け",
        "nameEn": "Dawn of the Final Battle",
        "desc": "全てを賭けた最後の戦い。夜明けと共に決着をつける",
        "bgmStyle": "フルオーケストラ / エピック・コーラス",
        "category": "ボスバトル",
        "audio_v1": "dawn_of_the_final_battle_v1.mp3",
        "image": "dawn_of_the_final_battle.png",
        "status": "ready"
      },
      {
        "id": "a_new_horizon",
        "name": "新たな地平",
        "nameEn": "A New Horizon",
        "desc": "戦いが終わり、新たな旅が始まる。希望に満ちた朝",
        "bgmStyle": "ポジティブ・オーケストラ / フルート",
        "category": "エンディング",
        "audio_v1": "a_new_horizon_v1.mp3",
        "audio_v2": "a_new_horizon_v2.mp3",
        "image": "a_new_horizon.png",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 10,
      "bgm": 19,
      "ready": 10
    }
  },
  "vlog-cafe": {
    "id": "vlog-cafe",
    "title": "Vlog / カフェ / 料理動画 BGM",
    "titleEn": "Vlog / Cafe / Cooking BGM",
    "description": "YouTube料理チャンネル、日常Vlog、ルーティン動画、カフェ系動画向け",
    "audio_folder": "vlog_cafe",
    "scenes": [
      {
        "id": "sunset_pasta",
        "name": "サンセットパスタ",
        "nameEn": "Sunset Pasta",
        "desc": "夕日に照らされたキッチンで作る手打ちパスタ",
        "bgmStyle": "アコースティックギター / ボサノバ",
        "category": "料理・キッチン",
        "audio_v1": "sunset_pasta_v1.mp3",
        "image": "sunset_pasta.png",
        "status": "ready"
      },
      {
        "id": "weekend_market",
        "name": "ウィークエンドマーケット",
        "nameEn": "Weekend Market",
        "desc": "週末の朝市で新鮮な食材を探す。賑やかで楽しい時間",
        "bgmStyle": "ウクレレ / ポップ",
        "category": "お出かけ",
        "audio_v1": "weekend_market_v1.mp3",
        "audio_v2": "weekend_market_v2.mp3",
        "image": "weekend_market.png",
        "status": "ready"
      },
      {
        "id": "dessert_night",
        "name": "デザートナイト",
        "nameEn": "Dessert Night",
        "desc": "夜のキッチンで作る特別なデザート。甘い香りが漂う",
        "bgmStyle": "ジャズピアノ / チル",
        "category": "料理・キッチン",
        "audio_v1": "dessert_night_v1.mp3",
        "audio_v2": "dessert_night_v2.mp3",
        "image": "dessert_night.png",
        "status": "ready"
      },
      {
        "id": "morning_smoothie",
        "name": "モーニングスムージー",
        "nameEn": "Morning Smoothie",
        "desc": "朝のルーティン。フルーツをミキサーにかける健康的な朝",
        "bgmStyle": "ブライト・ポップ / マリンバ",
        "category": "ルーティン",
        "audio_v1": "morning_smoothie_v1.mp3",
        "audio_v2": "morning_smoothie_v2.mp3",
        "image": "morning_smoothie.png",
        "status": "ready"
      },
      {
        "id": "afternoon_kitchen",
        "name": "アフタヌーンキッチン",
        "nameEn": "Afternoon Kitchen",
        "desc": "午後のゆったりとした時間。焼き菓子の香りが部屋に広がる",
        "bgmStyle": "ソフト・アコースティック",
        "category": "料理・キッチン",
        "audio_v1": "afternoon_kitchen_v1.mp3",
        "audio_v2": "afternoon_kitchen_v2.mp3",
        "image": "afternoon_kitchen.png",
        "status": "ready"
      },
      {
        "id": "window_seat_coffee",
        "name": "窓辺のコーヒー",
        "nameEn": "Window Seat Coffee",
        "desc": "窓辺の席でハンドドリップコーヒーを淹れる至福の時間",
        "bgmStyle": "Lo-Fi ジャズ / カフェ",
        "category": "カフェ",
        "audio_v1": "window_seat_coffee_v1.mp3",
        "audio_v2": "window_seat_coffee_v2.mp3",
        "image": "window_seat_coffee.png",
        "status": "ready"
      },
      {
        "id": "sunday_morning_routine",
        "name": "日曜の朝のルーティン",
        "nameEn": "Sunday Morning Routine",
        "desc": "休日の朝。ゆっくり起きてブランチを作る",
        "bgmStyle": "チルホップ / ピアノ",
        "category": "ルーティン",
        "audio_v1": "sunday_morning_routine_v1.mp3",
        "audio_v2": "sunday_morning_routine_v2.mp3",
        "image": "sunday_morning_routine.png",
        "status": "ready"
      },
      {
        "id": "city_walk",
        "name": "シティウォーク",
        "nameEn": "City Walk",
        "desc": "カメラを片手に街を散歩。お気に入りの場所を巡る",
        "bgmStyle": "インディーポップ / シンセ",
        "category": "お出かけ",
        "audio_v1": "city_walk_v1.mp3",
        "audio_v2": "city_walk_v2.mp3",
        "image": "city_walk.png",
        "status": "ready"
      },
      {
        "id": "first_apartment",
        "name": "はじめてのひとり暮らし",
        "nameEn": "First Apartment",
        "desc": "新生活の始まり。小さなキッチンで初めての自炊",
        "bgmStyle": "アコースティック / ポジティブ",
        "category": "ルーティン",
        "audio_v1": "first_apartment_v1.mp3",
        "audio_v2": "first_apartment_v2.mp3",
        "image": "first_apartment.png",
        "status": "ready"
      },
      {
        "id": "seoul_cafe_afternoon",
        "name": "ソウルカフェの午後",
        "nameEn": "Seoul Cafe Afternoon",
        "desc": "韓国風カフェでのんびり過ごす午後。おしゃれな空間",
        "bgmStyle": "K-インディー / ソフトR&B",
        "category": "カフェ",
        "audio_v1": "seoul_cafe_afternoon_v1.mp3",
        "audio_v2": "seoul_cafe_afternoon_v2.mp3",
        "image": "seoul_cafe_afternoon.png",
        "status": "ready"
      }
    ],
    "stats": {
      "total": 10,
      "bgm": 19,
      "ready": 10
    }
  }
,
  "hebi_santos": {
    "id": "hebi_santos",
    "title": "オープンワールド 都市RPG風 BGM",
    "titleEn": "Open World City RPG BGM Pack",
    "description": "GTAスタイルの都市ゲーム・RPドラマ・ストリート系動画向けBGM",
    "audio_folder": "hebi_santos",
    "image_subdir": "hebi_santos",
    "scenes": [
      {
        "id": "daily_bakery",
        "name": "朝のパン屋",
        "nameEn": "Morning Bakery",
        "desc": "温かい朝日が差し込むパン屋のキッチン。穏やかな日常シーン",
        "bgmStyle": "Lo-fi Hip Hop / インディーポップ",
        "category": "日常シーン",
        "image": "bakery_morning.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "daily_bakery_v1.mp3",
        "audio_v2": "daily_bakery_v2.mp3",
        "status": "ready"
      },
      {
        "id": "chiefs_office",
        "name": "署長室のエレガンス",
        "nameEn": "Chief's Office",
        "desc": "重厚なオフィスで繰り広げられる知的な駆け引き",
        "bgmStyle": "ジャズピアノ / ノワール",
        "category": "日常シーン",
        "image": "police_office.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "chiefs_office_v1.mp3",
        "audio_v2": "chiefs_office_v2.mp3",
        "status": "ready"
      },
      {
        "id": "empires_shadow",
        "name": "帝国の影",
        "nameEn": "Empire's Shadow",
        "desc": "巨大な権力が動き出す、威圧的なオーケストラ",
        "bgmStyle": "ダークオーケストラ / シネマティック",
        "category": "緊迫シーン",
        "image": "penthouse_night.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "empires_shadow_v1.mp3",
        "audio_v2": "empires_shadow_v2.mp3",
        "status": "ready"
      },
      {
        "id": "silk_and_poison",
        "name": "絹と毒",
        "nameEn": "Silk and Poison",
        "desc": "妖艶で危険な夜のラウンジ。裏の駆け引き",
        "bgmStyle": "トリップホップ / ダークR&B",
        "category": "緊迫シーン",
        "image": "dark_lounge.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "silk_and_poison_v1.mp3",
        "audio_v2": "silk_and_poison_v2.mp3",
        "status": "ready"
      },
      {
        "id": "drifting_green",
        "name": "漂う緑の霧",
        "nameEn": "Drifting Green",
        "desc": "霧に包まれた路地裏。超自然的な予兆",
        "bgmStyle": "アンビエント / ミステリアス",
        "category": "探索シーン",
        "image": "misty_alley.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "drifting_green_v1.mp3",
        "audio_v2": "drifting_green_v2.mp3",
        "status": "ready"
      },
      {
        "id": "silent_stall",
        "name": "港の屋台",
        "nameEn": "Harbor Stall",
        "desc": "薄暗い港の屋台。怪しげな取引の気配",
        "bgmStyle": "サスペンス / スリラー",
        "category": "探索シーン",
        "image": "harbor_stall.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "silent_stall_v1.mp3",
        "audio_v2": "silent_stall_v2.mp3",
        "status": "ready"
      },
      {
        "id": "taxi_cruise",
        "name": "ネオン街タクシー",
        "nameEn": "Neon City Taxi",
        "desc": "雨のネオン街をタクシーで疾走する夜",
        "bgmStyle": "シンセウェイブ / サイバーパンク",
        "category": "移動シーン",
        "image": "taxi_cityscape.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "taxi_cruise_v1.mp3",
        "audio_v2": "taxi_cruise_v2.mp3",
        "status": "ready"
      },
      {
        "id": "conspiracy_unfolds",
        "name": "陰謀の幕開け",
        "nameEn": "Conspiracy Unfolds",
        "desc": "暗い密室での秘密会議。真実が動き始める",
        "bgmStyle": "テンション / ノワール",
        "category": "緊迫シーン",
        "image": "conspiracy_room.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "conspiracy_unfolds_v1.mp3",
        "audio_v2": "conspiracy_unfolds_v2.mp3",
        "status": "ready"
      },
      {
        "id": "hebi_santos_radio",
        "name": "ラジオステーション",
        "nameEn": "Radio Station",
        "desc": "レトロな80年代風ラジオブース。Vice City的雰囲気",
        "bgmStyle": "シンセウェイブ / レトロ",
        "category": "日常シーン",
        "image": "radio_station.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "hebi_santos_radio_v1.mp3",
        "audio_v2": "hebi_santos_radio_v2.mp3",
        "status": "ready"
      },
      {
        "id": "tears_in_the_city",
        "name": "雨の街のメランコリー",
        "nameEn": "Rainy City Melancholy",
        "desc": "雨に濡れたネオン街のパノラマ。都市の孤独",
        "bgmStyle": "メランコリック / シネマティック",
        "category": "感傷シーン",
        "image": "rainy_city_night.png",
        "audio_folder": "hebi_santos",
        "image_subdir": "hebi_santos",
        "audio_v1": "tears_in_the_city_v1.mp3",
        "audio_v2": "tears_in_the_city_v2.mp3",
        "status": "ready"
      }
    ]
  },
  "sleep": {
    "id": "sleep",
    "title": "睡眠・ヒーリング BGM",
    "titleEn": "Sleep & Healing BGM Pack",
    "description": "安眠・リラクゼーション・ASMR系動画向けBGM",
    "audio_folder": "affiliate_sleep",
    "image_subdir": "affiliate_sleep",
    "scenes": [
      {
        "id": "deep_sleep_river",
        "name": "月夜の川辺",
        "nameEn": "Moonlit Riverbank",
        "desc": "月光が水面に反射する静かな川辺。蛍が舞う幻想的な夜",
        "bgmStyle": "ヒーリングアンビエント / 432Hz",
        "category": "睡眠導入",
        "image": "river_moonlight.png",
        "audio_folder": "affiliate_sleep",
        "image_subdir": "affiliate_sleep",
        "audio_v1": "deep_sleep_river_v1.mp3",
        "audio_v2": "deep_sleep_river_v2.mp3",
        "status": "ready"
      },
      {
        "id": "moonlit_forest_sleep",
        "name": "月明かりの森",
        "nameEn": "Moonlit Forest",
        "desc": "満月に照らされた神秘的な森。木々の間を漂う霧",
        "bgmStyle": "ネイチャーアンビエント / ドリーミー",
        "category": "睡眠導入",
        "image": "forest_night.png",
        "audio_folder": "affiliate_sleep",
        "image_subdir": "affiliate_sleep",
        "audio_v1": "moonlit_forest_sleep_v1.mp3",
        "audio_v2": "moonlit_forest_sleep_v2.mp3",
        "status": "ready"
      },
      {
        "id": "rain_on_window",
        "name": "窓の雨音",
        "nameEn": "Rain on Window",
        "desc": "雨粒がガラスを伝う夜。室内の温かなランプの灯り",
        "bgmStyle": "雨音アンビエント / ピアノ",
        "category": "睡眠導入",
        "image": "rain_window.png",
        "audio_folder": "affiliate_sleep",
        "image_subdir": "affiliate_sleep",
        "audio_v1": "rain_on_window_v1.mp3",
        "audio_v2": "rain_on_window_v2.mp3",
        "status": "ready"
      },
      {
        "id": "ocean_drift_sleep",
        "name": "星空の海",
        "nameEn": "Starlit Ocean",
        "desc": "満天の星空の下、穏やかな波が打ち寄せる砂浜",
        "bgmStyle": "海洋アンビエント / ディープリラックス",
        "category": "睡眠導入",
        "image": "ocean_night.png",
        "audio_folder": "affiliate_sleep",
        "image_subdir": "affiliate_sleep",
        "audio_v1": "ocean_drift_sleep_v1.mp3",
        "audio_v2": "ocean_drift_sleep_v2.mp3",
        "status": "ready"
      },
      {
        "id": "baby_lullaby_dreams",
        "name": "ベビーララバイ",
        "nameEn": "Baby Lullaby Dreams",
        "desc": "優しいオルゴールの旋律。赤ちゃんの安眠に",
        "bgmStyle": "オルゴール / ララバイ",
        "category": "子守唄",
        "image": "river_moonlight.png",
        "audio_folder": "affiliate_sleep",
        "image_subdir": "affiliate_sleep",
        "audio_v1": "baby_lullaby_dreams_v1.mp3",
        "audio_v2": "baby_lullaby_dreams_v2.mp3",
        "status": "ready"
      }
    ]
  },
  "focus": {
    "id": "focus",
    "title": "集中・作業用 BGM",
    "titleEn": "Focus & Productivity BGM Pack",
    "description": "勉強・プログラミング・デスクワーク向けの集中BGM",
    "audio_folder": "affiliate_focus",
    "image_subdir": "affiliate_focus",
    "scenes": [
      {
        "id": "focus_zone_alpha",
        "name": "集中ゾーン・アルファ",
        "nameEn": "Focus Zone Alpha",
        "desc": "バイノーラルビートとミニマル電子音。集中力を高める環境音",
        "bgmStyle": "バイノーラルビート / フォーカスアンビエント",
        "category": "集中",
        "image": "coding_desk.png",
        "audio_folder": "affiliate_focus",
        "image_subdir": "affiliate_focus",
        "audio_v1": "focus_zone_alpha_v1.mp3",
        "audio_v2": "focus_zone_alpha_v2.mp3",
        "status": "ready"
      },
      {
        "id": "cafe_study_session",
        "name": "カフェ勉強セッション",
        "nameEn": "Cafe Study Session",
        "desc": "おしゃれなカフェの雑踏音とジャズピアノ。リラックスした勉強環境",
        "bgmStyle": "カフェアンビエント / ジャズピアノ",
        "category": "集中",
        "image": "study_cafe.png",
        "audio_folder": "affiliate_focus",
        "image_subdir": "affiliate_focus",
        "audio_v1": "cafe_study_session_v1.mp3",
        "audio_v2": "cafe_study_session_v2.mp3",
        "status": "ready"
      },
      {
        "id": "midnight_coding",
        "name": "深夜コーディング",
        "nameEn": "Midnight Coding",
        "desc": "モニターの光だけが照らすデスク。深夜のプログラミング",
        "bgmStyle": "ダークアンビエント / テクノ",
        "category": "集中",
        "image": "coding_desk.png",
        "audio_folder": "affiliate_focus",
        "image_subdir": "affiliate_focus",
        "audio_v1": "midnight_coding_v1.mp3",
        "audio_v2": "midnight_coding_v2.mp3",
        "status": "ready"
      },
      {
        "id": "study_beats_lofi",
        "name": "勉強用ローファイ",
        "nameEn": "Study Beats Lo-fi",
        "desc": "定番のLo-fiヒップホップ。レコードノイズと暖かいキー",
        "bgmStyle": "Lo-fi Hip Hop / チルビーツ",
        "category": "集中",
        "image": "lofi_room.png",
        "audio_folder": "affiliate_focus",
        "image_subdir": "affiliate_focus",
        "audio_v1": "study_beats_lofi_v1.mp3",
        "audio_v2": "study_beats_lofi_v2.mp3",
        "status": "ready"
      },
      {
        "id": "productivity_timer",
        "name": "プロダクティビティ・タイマー",
        "nameEn": "Productivity Timer",
        "desc": "ポモドーロ・テクニック用のミニマルアンビエント",
        "bgmStyle": "ミニマルアンビエント / ホワイトノイズ",
        "category": "集中",
        "image": "library_corner.png",
        "audio_folder": "affiliate_focus",
        "image_subdir": "affiliate_focus",
        "audio_v1": "productivity_timer_v1.mp3",
        "audio_v2": "productivity_timer_v2.mp3",
        "status": "ready"
      }
    ]
  },
  "relax": {
    "id": "relax",
    "title": "リラックス・癒し BGM",
    "titleEn": "Relaxation & Healing BGM Pack",
    "description": "ヨガ・瞑想・入浴・読書タイム向けのリラクゼーションBGM",
    "audio_folder": "affiliate_relax",
    "image_subdir": "affiliate_relax",
    "scenes": [
      {
        "id": "morning_yoga_flow",
        "name": "朝ヨガフロー",
        "nameEn": "Morning Yoga Flow",
        "desc": "丘の上で迎える朝日。心身を整える穏やかな時間",
        "bgmStyle": "ニューエイジ / シンギングボウル",
        "category": "ヨガ・瞑想",
        "image": "yoga_sunrise.png",
        "audio_folder": "affiliate_relax",
        "image_subdir": "affiliate_relax",
        "audio_v1": "morning_yoga_flow_v1.mp3",
        "audio_v2": "morning_yoga_flow_v2.mp3",
        "status": "ready"
      },
      {
        "id": "hot_spring_healing",
        "name": "温泉ヒーリング",
        "nameEn": "Onsen Healing",
        "desc": "竹林に囲まれた露天風呂。湯気と石灯篭の灯り",
        "bgmStyle": "和風アンビエント / 琴",
        "category": "ヨガ・瞑想",
        "image": "onsen_zen.png",
        "audio_folder": "affiliate_relax",
        "image_subdir": "affiliate_relax",
        "audio_v1": "hot_spring_healing_v1.mp3",
        "audio_v2": "hot_spring_healing_v2.mp3",
        "status": "ready"
      },
      {
        "id": "fireplace_reading",
        "name": "暖炉と読書",
        "nameEn": "Fireplace Reading",
        "desc": "パチパチと燃える暖炉のそばで、ウール毛布にくるまる読書時間",
        "bgmStyle": "アコースティック / ヒュッゲ",
        "category": "くつろぎ",
        "image": "fireplace_reading.png",
        "audio_folder": "affiliate_relax",
        "image_subdir": "affiliate_relax",
        "audio_v1": "fireplace_reading_v1.mp3",
        "audio_v2": "fireplace_reading_v2.mp3",
        "status": "ready"
      },
      {
        "id": "desk_workout_energy",
        "name": "デスクワークアウト",
        "nameEn": "Desk Workout Energy",
        "desc": "軽いストレッチやデスクエクササイズに。前向きなエネルギー",
        "bgmStyle": "アップリフティング / ポップ",
        "category": "エクササイズ",
        "image": "yoga_sunrise.png",
        "audio_folder": "affiliate_relax",
        "image_subdir": "affiliate_relax",
        "audio_v1": "desk_workout_energy_v1.mp3",
        "audio_v2": "desk_workout_energy_v2.mp3",
        "status": "ready"
      },
      {
        "id": "night_drive_ambient",
        "name": "ナイトドライブ",
        "nameEn": "Night Drive Ambient",
        "desc": "深夜のハイウェイ。シンセウェイブの夕焼けとネオンの光",
        "bgmStyle": "チルシンセウェイブ / レトロウェイブ",
        "category": "くつろぎ",
        "image": "night_drive.png",
        "audio_folder": "affiliate_relax",
        "image_subdir": "affiliate_relax",
        "audio_v1": "night_drive_ambient_v1.mp3",
        "audio_v2": "night_drive_ambient_v2.mp3",
        "status": "ready"
      }
    ]
  }
};
