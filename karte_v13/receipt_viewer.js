      ...
      case 'IY':
        if (currentReceipt) {
          ...
          currentReceipt.procedures.push({
            ...
            quantity: parseFloat(fields[4]) || 0,
            days: iyDays,
            isDrug: true,
            _raw: line
          });
...
function checkReceipt(r) {
  ...
  // 4. 3層整合チェック（記録条件仕様のお手本に基づく／v0.13）
  //   第1層: 請求合計(HO[5]/KO[5]) と 明細点数の単純和
  //     ※ レセ電の点数は加算・逓減・包括が絡むため一致しないのが正常。エラー判定にはせず情報表示に留める。
  //       明細から点数を1つも取得できなかった=パース不全のみを警告する。
...
function renderIntegrityPanel(r) {
  ...
  const diff = ig.diff;
  const diffOk = diff === 0;
  const diffColor = diffOk ? 'var(--rc-green)' : 'var(--rc-amber)';
  const diffMark = diffOk ? '&#10003; 一致' :
    (ig.calcPoints === 0 ? '明細点数なし' : '差' + diff.toLocaleString() + '点（加算・逓減・包括による正常差の可能性）');
  ...
  return (
    '<div style="margin-top:10px;border:1px solid var(--rc-border,#d9d4c8);border-left:4px solid var(--rc-teal,#0e7c66);background:rgba(14,124,102,0.04);padding:8px 12px;font-size:12px;line-height:1.7;">' +
      ...
      '<div><span style="display:inline-block;min-width:8.5em;color:#666;">① 請求合計 vs 明細和</span>' +
        '<strong>' + ig.billTotal.toLocaleString() + '点</strong> ／ 単純和 ' + ig.calcPoints.toLocaleString() + '点 ' +
