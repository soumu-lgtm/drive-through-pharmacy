...
function checkReceipt(r) {
  ...
  // 大きくずれている場合のみ警告（コード内蔵点数があるため完全一致は無理）
  // 点数辞書に含まれないコードが多いため、この差異チェックは無効化
  // 将来的に完全な点数マスタを導入したら復活
  // if (r.totalPoints > 0 && calcPoints > 0 && Math.abs(r.totalPoints - calcPoints) > r.totalPoints * 0.5) {
  //   r.warnings.push({ severity: 'low', message: '合計点数に大きな差異: 請求=' + r.totalPoints + ' 計算=' + calcPoints });
  // }
